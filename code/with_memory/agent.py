...
OBSERVE_MAX=30	# Max number of no-op (no operation) steps taken at the beginning of each game episode.
...
MINIBATCH_SIZE=32 # Size of the minibatches that are given to the network for one update.
...
# Where to save the training output. If None, the output is printed on stdout.
...
def printmsg(msg):
	"""
	For printing the training output.
	"""
	if(output_path):
...
class ExperienceReplay():
	...
	def addTuple(self, state, action, reward, nextState, terminal):
		"""
		Adds the tuple to the memory, effectively discarding the oldest entry if the memory is full.
		"""
		global TEST_SET
	...
	def getMiniBatch(self, sampleSize=MINIBATCH_SIZE):
		"""
		Returns a sampled minibatch of states, in a divided tuple.
		"""
		minibatch=random.sample(self.memory, sampleSize) # an array of tuples
